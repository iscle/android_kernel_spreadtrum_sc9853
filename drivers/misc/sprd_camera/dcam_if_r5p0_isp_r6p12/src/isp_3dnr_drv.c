...
static int isp_3dnr_get_memctrl_param(struct isp_nr3_param *param,
...
static int isp_3dnr_get_nr3store_param(struct isp_nr3_param *param,
	struct camera_frame *frame)
{
...
static int isp_3dnr_get_crop_param(struct isp_nr3_param *param,
	struct camera_frame *frame)
{
	enum isp_drv_rtn ret = ISP_RTN_SUCCESS;
...
int isp_3dnr_get_param(struct isp_nr3_param *param,
	void *frame)
{
	enum isp_drv_rtn ret = ISP_RTN_SUCCESS;
	...
	ret = isp_3dnr_get_memctrl_param(param, frame);
	if (ret) {
		pr_err("fail to 3ndr get memctrl\n");
		return ret;
	}

	ret = isp_3dnr_get_nr3store_param(param, frame);
	if (ret) {
		pr_err("fail to 3ndr get nr3store\n");
		return ret;
	}

	ret = isp_3dnr_get_crop_param(param, frame);
	if (ret) {
		pr_err("fail to 3ndr get nr3store\n");
		return ret;
	}
...
static int mv_conversion_base_on_resulution(int mv_x, int mv_y,
...
int isp_3dnr_conversion_mv(struct isp_nr3_param *nr3_param,
	void *pdata)
{
	int input_x = 0;
	int input_y = 0;
	...
	ret = mv_conversion_base_on_resulution(input_x,
			input_y,
...
int isp_3dnr_release(void *isp_handle)
{
