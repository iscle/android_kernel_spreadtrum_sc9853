...
static enum tfa_error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int *vstep)
{
	...
	pr_err("----------dll test -----in tfa98xx_tfa_start\n");
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (!strncmp(buf, start_cmd, sizeof(start_cmd) - 1)) {
		...
		pr_err("----------dll test -----in tfa98xx_dbgfs_dsp_state_set start\n");
		mutex_lock(&tfa98xx->dsp_lock);
		...
		pr_err("----------dll_test--------tfa98xx_dbgfs_dsp_state_set\n");
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_err("%ld\n", ucontrol->value.integer.value[0]);
	pr_err("----------dll_test--------tfa98xx_set_stop_ctl\n");
	tfa98xx_open(tfa98xx->handle);
	tfa98xx_dsp_system_stable(tfa98xx->handle, &ready);
	tfa98xx_close(tfa98xx->handle);
 ...
 */
static char *fw_name = "tfa9897.cnt";
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		 ...
		 * are deactivated
		 */
		pr_err("----------dll_test--------tfa_mute in tfa98xx_mute\n");
...
static ssize_t tfa98xx_chip_exist_show(struct kobject *kobj,
				       struct kobj_attribute *attr, char *buff)
{
	return sprintf(buff, "%d\n", !no_chip);
}

static int tfa98xx_sysfs_create_chip_exist(void)
{
	int ret;
	static struct kobj_attribute chip_exit =
		__ATTR(ext_smartpa_exist, 0644,
		tfa98xx_chip_exist_show,
		NULL);
...
static int tfa98xx_dummy_process(struct i2c_client *i2c)
{
	int ret;
	struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);

	ret = snd_soc_register_codec(&i2c->dev, &soc_codec_dev_tfa98xx,
				     tfa98xx_dai, ARRAY_SIZE(tfa98xx_dai));
	if (ret < 0) {
		dev_err(&i2c->dev, "Failed to register TFA98xx for dummy codec: %d\n",
			ret);
		return ret;
	}
			     ...
			     const struct i2c_device_id *id)
{
	...
	tfa98xx_sysfs_create_chip_exist();
	...
	if (no_start == 0) {
		...
		if (ret < 0) {
			dev_err(&i2c->dev, "Failed to read Revision register: %d\n",
				ret);
			no_chip = true;
			return tfa98xx_dummy_process(i2c);
		}
		switch (reg & 0xff) {
		case 0x72: /* tfa9872 */
