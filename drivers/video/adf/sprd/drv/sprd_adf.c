...
size_t sprd_adf_interface_get_modes(struct sprd_adf_interface *intf,
			struct drm_mode_modeinfo *mode, size_t n_modes)
{
	...
	for (i = 1; i < n_modes; i++) {
		res = panel->sr_table[i - 1];
		mode[i].hdisplay = res & 0xFFFF;
		mode[i].vdisplay = res >> 16;
