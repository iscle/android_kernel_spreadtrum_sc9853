...
struct camera_path_spec {
	...
	struct cam_buf_queue tmp_buf_queue;
...
struct camera_info {
	...
	uint32_t scene_mode;
	uint32_t raw_cap_flag;
	uint32_t slow_motion_flag;
...
struct camera_dev {
	...
	struct camera_info cam_ctx;/*store info from user ioctl*/
	uint32_t use_path;
	atomic_t stream_on;
	atomic_t run_flag;
...
static int sprd_deinit_handle(struct camera_dev *dev)
{
	...
	info->need_isp_tool = 0;
	atomic_set(&dev->stream_on, 0);

	pr_info("sprd_img: deinit handle end!\n");
	return 0;
}

static int sprd_camera_dev_init(struct camera_group *group,
	enum dcam_id idx)
{
	int ret = 0;
	...
	ret = sprd_cam_buf_queue_init(&dev->queue, CAM_QUEUE_RW_LOCK,
	...
	sprd_init_timer(&dev->cam_timer, (unsigned long)dev);

	ret  = dcam_create_flash_task(&dev->flash_task, idx);
	if (unlikely(ret != 0)) {
...
isp_exit:
	dcam_destroy_flash_task(dev->flash_task);
flash_exit:
	sprd_stop_timer(&dev->cam_timer);
	sprd_cam_buf_queue_deinit(&dev->queue);
queue_exit:
	sprd_deinit_handle(dev);
users_exit:
	vfree(dev);
vzalloc_exit:
	return ret;
...
static void sprd_camera_dev_deinit(struct camera_group *group,
	enum dcam_id idx)
{
	struct camera_dev *dev = NULL;
	...
	if (idx != DCAM_ID_2)
		sprd_isp_dev_deinit(dev->isp_dev_handle, (int)idx);
...
static int sprd_img_k_open(struct inode *node, struct file *file)
{
	...
	if (count == 0 || count > DCAM_MAX_COUNT) {
	...
	if (atomic_read(&grp->camera_opened) == 1) {
		...
		ret = sprd_camera_dev_init(grp, DCAM_ID_0);
		if (unlikely(ret != 0)) {
		...
		if (count > 1) {
			ret = sprd_camera_dev_init(grp, DCAM_ID_1);
			if (unlikely(ret != 0)) {
		...
		if (count > 2) {
			ret = sprd_camera_dev_init(grp, DCAM_ID_2);
			if (unlikely(ret != 0)) {
...
dev2_fail:
	sprd_camera_dev_deinit(camerafile->grp, DCAM_ID_1);
dev1_fail:
	...
	if (atomic_dec_return(&group->camera_opened) == 0) {
		for (i = 0; i < group->dcam_count; i++) {
			...
			if (group->dev_inited & (1 << 0))
				complete(&group->dev[i]->irq_com);
			sprd_camera_stream_off(camerafile, group->dev[i], 0);
			...
			unsigned long arg)
{
	...
	ret = sprd_img_get_cam_dev(camerafile, &dev, &info);
	if (ret) {
			...
			size_t cnt, loff_t *cnt_ret)
{
	...
	if (!dev) {
		pr_info("fail to get inited dev %p\n", dev);
		return -EFAULT;
	}

	switch (read_op.cmd) {
	case SPRD_IMG_GET_SCALE_CAP:
		CAM_TRACE("get scale capbility\n");
	...
	case SPRD_IMG_GET_FRM_BUFFER:
		...
		if (sprd_cam_buf_queue_read(&dev->queue, &node)) {
		...
		if (read_op.evt == IMG_TX_DONE) {
			...
			path =
				&dev->cam_ctx.cam_path[read_op.parm.frame.
				channel_id];
			CAM_TRACE("node, %p %d 0x%x,channel_id %d\n",
				(void *)&node, node.index, path->frm_id_base,
		...
		if (read_op.parm.frame.irq_property == IRQ_RAW_CAP_DONE) {
			...
			if (dev->raw_cap == 1) {
				dev->raw_cap = 0;
				sprd_dcam_stop_fetch(dev->idx, dev->idx, group);
				ret = sprd_cam_buf_queue_clear
					(&dev->queue);
				if (unlikely(ret != 0))
					pr_err("fail to init queue\n");
			...
			size_t cnt, loff_t *cnt_ret)
{
	...
	case SPRD_IMG_FREE_FRAME:
		...
		case CAMERA_CAP_PATH:
			path = &info->cam_path[write_op.channel_id];
			break;
		default:
		...
		if (path->status == PATH_IDLE) {
			CAM_TRACE("fail to free frame buf, channel %d\n",
				write_op.channel_id);
			return -EINVAL;
		}
		...
		if (write_op.index >
			...
			CAM_TRACE("free frame buf, channel %d, index 0x%x\n",
...
static void sprd_img_shutdown(struct platform_device *pdev)
{
	...
	dcam_set_flash(DCAM_ID_0, &set_flash);
