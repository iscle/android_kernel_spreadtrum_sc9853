...
void isp_frm_clear(struct isp_pipe_dev *dev, enum isp_path_index path_index)
{
	...
	if (ISP_PATH_IDX_PRE & path_index) {
		...
		while (!sprd_cam_frm_dequeue(&path->frame_queue, &frame)) {
			cam_buf_unmap_addr(&frame.buf_info);
			memset(&frame, 0x00, sizeof(frame));
		}

		sprd_cam_frm_queue_clear(&path->frame_queue);
		ret = sprd_cam_buf_queue_clear(&path->buf_queue);
		if (unlikely(ret != 0))
			pr_err("fail to clear queue\n");
		ret = sprd_cam_buf_queue_clear(&path->coeff_queue);
		if (unlikely(ret != 0))
...
static void isp_cfg_buf_int(struct isp_cfg_ctx_desc *cfg_ctx,
			void *phy_addr, void *vir_addr)
{
	int scene_id = 0, num = 0, id = cfg_ctx->cur_isp_id;
	...
	for (scene_id = 0; scene_id < ISP_SCENE_MAX; scene_id++) {
		...
		for (num = 0; num < ISP_CFG_BUF_NUM; num++) {
			...
			pr_info("cfg_cmd%d_%d:hw=0x%lx, sw=0x%lx\n",
			id, scene_id,
			(unsigned long)ion_buf->cmd_buf[num].phy_addr,
			(unsigned long)ion_buf->cmd_buf[num].vir_addr);
		}
	}
}
...
int isp_cfg_ctx_buf_iommu_map(struct isp_cfg_ctx_desc *cfg_ctx)
{
	...
	rtn = cam_buf_map_addr(&cfg_ctx->buf_info);
	if (rtn) {
	...
	isp_cfg_buf_int(cfg_ctx, phy_addr, vir_addr);
	return rtn;
}
