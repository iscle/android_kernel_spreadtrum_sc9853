...
static int cpp_module_disable(struct cpp_device *dev)
{
	...
	if (atomic_dec_return(&dev->pw_users) == 0) {
		/* we will first check if cpp is still
		 * working with ISP
		*/

#ifdef CONFIG_ISP_CPP_COWORK_SUPPORT
		if (sprd_cpp_dev && sprd_cpp_dev->is_cowork_isp_running) {
			pr_info("stopping the isp_cpp cowork\n");
			if (sprd_cpp_dev->isp_dev)
				isp_cpp_deinit(sprd_cpp_dev->isp_dev);
		}
#endif
		/* disable clk */
		regmap_update_bits(dev->cam_ahb_gpr,
...
int sprd_cpp_set_isp_dev(void *dev)
{
	if (dev == NULL)
		return -EINVAL;
	sprd_cpp_dev->isp_dev = dev;
	return 0;
}
#endif
...
#ifdef CONFIG_ISP_CPP_COWORK_SUPPORT
	sprd_cpp_dev->is_cowork_isp_running = 0;
	sprd_cpp_dev->isp_dev = NULL;
#endif
