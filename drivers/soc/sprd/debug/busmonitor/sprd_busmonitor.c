...
static int sprd_busmon_config_init(struct sprd_busmonitor *bm, u32 n)
{
	...
	const char *propname[] = { "sprd,bm-config", "sprd,bm-id",
				"sprd,bm-ranges" };
...
static DEVICE_ATTR(busmonitor, S_IRUSR | S_IRGRP, sprd_busmon_show, NULL);
static DEVICE_ATTR(dump_scene, S_IRUSR | S_IRGRP,
		   sprd_busmon_dump_show, NULL);
static DEVICE_ATTR(panic, S_IRUSR | S_IRGRP, sprd_busmon_panic_show, NULL);
static DEVICE_ATTR(active, S_IWUSR | S_IRUGO, NULL,
		   sprd_busmon_active_store);
static DEVICE_ATTR(config, S_IWUSR | S_IRUGO, sprd_busmon_cfg_show,
...
static int sprd_busmon_probe(struct djtag_device *ddev)
{
	...
	bm->irq = of_irq_get(ddev->dev.of_node, 0);
	if (bm->irq < 0)
		return bm->irq;
	...
	ret = devm_request_threaded_irq(&ddev->dev, bm->irq,
					sprd_busmon_irq,
					NULL, IRQF_TRIGGER_NONE,
					"aon,busmonitor", bm);
	if (ret) {
		dev_err(&ddev->dev, "can't request irq\n");
		return ret;
	}
	...
	dev_dbg(bm->dev, "sprd_bm_probe is ok\n");
