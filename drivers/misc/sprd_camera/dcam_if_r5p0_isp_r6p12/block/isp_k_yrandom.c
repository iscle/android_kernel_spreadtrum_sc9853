...
static int isp_k_yrandom_block(struct isp_io_param *param, enum isp_id idx)
{
	...
	if (ret != 0) {
		pr_err("isp_k_yrandom_block: copy_from_user error, ret = 0x%x\n",
			(unsigned int)ret);
		return -1;
	}
...
int isp_k_cfg_yrandom(struct isp_io_param *param, enum isp_id idx)
{
	...
	if (!param) {
		pr_err("isp_k_cfg_yrandom: param is null error.\n");
		return -1;
	}

	if (param->property_param == NULL) {
		pr_err("isp_k_cfg_yrandom: property_param is null error.\n");
		return -1;
	}
