...
static int isp_k_ynr_block(struct isp_io_param *param, uint32_t idx)
{
	int ret = 0;
	unsigned int val = 0;
...
static int isp_k_ynr_slice(struct isp_io_param *param, uint32_t idx)
{
	...
	if (ret != 0) {
		pr_err("isp_k_ynr_slice: copy_from_user error, ret = 0x%x\n",
			(unsigned int)ret);
		return -1;
	}
		...
		struct isp_k_block *isp_k_param, uint32_t com_idx)
{
	...
	case ISP_PRO_YNR_BLOCK:
		ret = isp_k_ynr_block(param, com_idx);
		break;
