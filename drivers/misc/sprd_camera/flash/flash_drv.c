...
int sprd_flash_ctrl(struct sprd_img_set_flash *set_flash)
{
	...
	case FLASH_CLOSE:
		if (!flash_dev->flashlight_status)
				...
				size_t size)
{
	...
	pr_info("cmd:%d flash:%d element.index %d\n",
			cmd, led_idx, element.index);

	switch (cmd) {
	case 0:
		flash_dev->flashlight_status = 1;
		flash_open_torch(flash_dev, 0, led_idx);
		break;
	case 1:
		flash_dev->flashlight_status = 0;
		flash_close_torch(flash_dev, 0, led_idx);
		break;
	case 2:
		flash_open_preflash(flash_dev, 0, led_idx);
		break;
	case 3:
		flash_close_preflash(flash_dev, 0, led_idx);
		break;
	case 4:
		flash_open_highlight(flash_dev, 0, led_idx);
		break;
	case 5:
		flash_close_highlight(flash_dev, 0, led_idx);
		break;
	case 6:
		flash_cfg_value_torch(flash_dev, 0, led_idx, &element);
		break;
	case 7:
		flash_cfg_value_preflash(flash_dev, 0, led_idx, &element);
		break;
	case 8:
		flash_cfg_value_highlight(flash_dev, 0, led_idx, &element);
		break;
	default:
		break;
	}
