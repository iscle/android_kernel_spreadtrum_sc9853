...
#ifdef CONFIG_ISP_CPP_COWORK_SUPPORT
static int isp_ion_cpp_buf_iommu_map(
	struct device *dev, struct path_cpp_ion_buf *ion_buf,
	struct camera_frame *frame)
{
	enum isp_rtn rtn = ISP_RTN_SUCCESS;
	struct sprd_iommu_map_data iommu_data;
	...
	memset(&iommu_data, 0, sizeof(struct sprd_iommu_map_data));
	pr_info("isp cpp map size 0x%zx\n", ionbuffer->size);
	...
	frame->yaddr = iommu_data.iova_addr;
	pr_debug("isp cpp map phys_yaddr 0x%x\n", frame->yaddr);
	frame->uaddr = frame->yaddr + frame->pfinfo.size[0];
	pr_debug("isp_cpp ion buf_iommu_map\n");
	return rtn;
}

static int isp_ion_cpp_buf_iommu_unmap(struct device *dev,
		struct path_cpp_ion_buf *ion_buf,
		unsigned int addr)
{
	struct ion_buffer *ionbuffer = NULL;
	struct sprd_iommu_unmap_data iommu_data;
	...
	ionbuffer = ion_handle_buffer(ion_buf->y_handle);
	pr_info("isp cpp unmap addr 0x%x size 0x%zx\n",
			addr, ionbuffer->size);
	memset(&iommu_data, 0, sizeof(struct sprd_iommu_unmap_data));
	iommu_data.iova_size = ionbuffer->size;
	iommu_data.ch_type = SPRD_IOMMU_FM_CH_RW;
	iommu_data.iova_addr = addr;
	iommu_data.buf = NULL;
	rtn = sprd_iommu_unmap(dev, &iommu_data);
	pr_debug("isp cpp unmamp rtn %d", rtn);
	return rtn;
}
		...
		int idx)
{
	...
	pr_info("isp cpp buffer alloc size 0x%zx\n", size);
	if (IS_ERR(isp_dev)) {
	...
	for (num = 0; num < PATH_CPP_ION_BUF_NUM; num++) {
		...
		isp_ion_cpp_buf_iommu_map(isp_dev, ion_buf,
					&frame);
		...
		isp_ion_cpp_buf_iommu_map(cpp_dev, ion_buf,
					&frame);
...
int isp_cpp_clear_buf(int idx, struct isp_cpp_desc *isp_cpp_dev)
{
	...
	for (num = 0; num < PATH_CPP_ION_BUF_NUM; num++) {
		...
		if (ion_buf->y_client == NULL || ion_buf->y_handle == NULL)
			return -EPERM;
		/*umap the isp iommu*/
		isp_ion_cpp_buf_iommu_unmap(isp_dev, ion_buf,
					ion_buf->isp_addr.yaddr);
		/*umap the cpp iommu*/
		isp_ion_cpp_buf_iommu_unmap(cpp_dev, ion_buf,
					ion_buf->cpp_addr.yaddr);
		ion_free(ion_buf->y_client, ion_buf->y_handle);
		ion_client_destroy(ion_buf->y_client);
		ion_buf->y_client = NULL;
		ion_buf->y_handle = NULL;
