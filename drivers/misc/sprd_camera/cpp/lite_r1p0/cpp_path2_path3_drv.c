...
#define CPP_PATH3_DES_UV_DOWN_SHRINK_THRESHOLD 16
#define CPP_PATH3_DES_UV_UP_SHRINK_THRESHOLD 240
#define CPP_PATH3_DES_Y_DOWN_SHRINK_THRESHOLD 16
#define CPP_PATH3_DES_Y_UP_SHRINK_THRESHOLD 235
#define CPP_PATH3_DEST_Y_SHRINK_OFFSET 16
#define CPP_PATH3_DEST_Y_SHRINK_RANGE 2
#define CPP_PATH3_DEST_UV_SHRINK_OFFSET 16
#define CPP_PATH3_DEST_UV_SHRINK_RANGE 6
...
{
	int ret = 0;
	unsigned int uv_down_thr;
	unsigned int uv_up_thr;
	unsigned int y_down_thr;
	unsigned int y_up_thr;
	unsigned int y_shrink_offset;
	unsigned int y_shrink_range;
	unsigned int uv_shrink_offset;
	unsigned int uv_shrink_range;
	unsigned int val = 0;
...
#ifdef CONFIG_ISP_CPP_COWORK_SUPPORT
	if (p->is_from_isp) {
		uv_down_thr = CPP_PATH3_DES_UV_DOWN_SHRINK_THRESHOLD;
		uv_up_thr = CPP_PATH3_DES_UV_UP_SHRINK_THRESHOLD;
		y_down_thr = CPP_PATH3_DES_Y_DOWN_SHRINK_THRESHOLD;
		y_up_thr = CPP_PATH3_DES_Y_UP_SHRINK_THRESHOLD;
		y_shrink_offset = CPP_PATH3_DEST_Y_SHRINK_OFFSET;
		y_shrink_range = CPP_PATH3_DEST_Y_SHRINK_RANGE;
		uv_shrink_offset = CPP_PATH3_DEST_UV_SHRINK_OFFSET;
		uv_shrink_range = CPP_PATH3_DEST_UV_SHRINK_RANGE;

		val = ((uv_down_thr & 0xff) << 8) |
				((uv_up_thr & 0xff) << 16);
		reg_wr(p, CPP_PATH3_DES_YUV_REGULATE_1, val);

		val = ((y_down_thr & 0xff) << 16) |
				((y_up_thr & 0xff) << 24);
		reg_wr(p, CPP_PATH3_DES_YUV_REGULATE_0, val);

		val = ((y_shrink_offset & 0x1f) << 4) |
				((y_shrink_range & 0xf) << 0);
		reg_wr(p, CPP_PATH3_DES_YUV_REGULATE_2, val);

		val = ((uv_shrink_offset & 0x1f) << 20) |
				((uv_shrink_range & 0xf) << 16);
		reg_owr(p, CPP_PATH3_DES_YUV_REGULATE_2, val);

		/*enable shrink mode for path3*/
		reg_owr(p, CPP_PATH3_DES_YUV_REGULATE_0, 0x1);
	}
#endif
