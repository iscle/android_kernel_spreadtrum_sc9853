...
static u32 wcn_get_sleep_status(void)
{
	struct wcn_device *wcn_dev;
	u32 sleep_status = 0;
...
static int wcn_proc_native_start(void *arg)
{
	...
	if (is_marlin) {
		...
		for (wait_count = 0; wait_count < MARLIN_WAIT_CP_INIT_COUNT;
			...
			msleep(MARLIN_WAIT_CP_INIT_POLL_TIME_MS);
			WCN_INFO("BTWF: magic_value=%d, wait_count=%d\n",
				magic_value, wait_count);
		}
		...
		for (wait_count = 0; wait_count < GNSS_WAIT_CP_INIT_COUNT;
			...
			WCN_INFO("GPS: magic_value=%d, wait_count=%d\n",
				magic_value, wait_count);
		}
	}
...
static void wcn_power_wq(struct work_struct *pwork)
{
	...
	WCN_INFO("work queue start wcn_dev name :%s\n",
		wcn_dev->name);
...
static int start_integrate_wcn_truely(u32 subsys)
{
	...
	if ((wcn_dev->wcn_open_status & WCN_MARLIN_MASK)
		...
		WCN_INFO("marlin opened already = %d, subsys=%d!\n",
			wcn_dev->wcn_open_status, subsys);
		return 0;
		...
		wcn_dev->wcn_open_status |= subsys_bit;
		mutex_unlock(&(wcn_dev->power_lock));
		WCN_INFO("gnss opened already = %d, subsys=%d!\n",
			wcn_dev->wcn_open_status, subsys);
		return 0;
	}
...
static int stop_integrate_wcn_truely(u32 subsys)
{
	...
	if (subsys_bit & WCN_MARLIN_MASK)
		...
		WCN_ERR("sub sys invalid: subsys=%d!\n", subsys);
		return -EINVAL;
	}
	...
	if ((wcn_dev->wcn_open_status & WCN_MARLIN_MASK)
		...
		WCN_INFO("open_status = %d,power_state=%d,stop subsys=%d!\n",
	...
	WCN_ERR("subsys = %d, and %s is not open!\n",
	       subsys, wcn_dev->name);
	mutex_unlock(&(wcn_dev->power_lock));
