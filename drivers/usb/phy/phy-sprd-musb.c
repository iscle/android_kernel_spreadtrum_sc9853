...
static int sprd_hsphy_init(struct usb_phy *x)
{
	...
	efuse_val = sprd_ap_efuse_read(phy->efuse_blk_id);
#elif defined(CONFIG_OTP_SPRD_AP_PUBLIC_EFUSE)
	efuse_val = sprd_efuse_double_read(phy->efuse_blk_id,
				phy->efuse_backup);
#else
	efuse_val = 0;
#endif
	if ((efuse_val & OTG_PHY_TUNE_EFUSE_FLG) == 0) {
		reg1 = readl_relaxed(PHY_REG_BASE + REG_AP_AHB_OTG_CTRL0);
		reg1 &= ~BIT_AP_AHB_USB20_TUNEOTG(7);
		writel_relaxed(reg1, PHY_REG_BASE + REG_AP_AHB_OTG_CTRL0);

		reg1 = readl_relaxed(PHY_REG_BASE + REG_AP_AHB_OTG_CTRL1);
		reg1 &= ~BIT_AP_AHB_USB20_TFREGRES(0x3F);
		reg1 |= BIT_AP_AHB_USB20_TFREGRES(0x07);
		reg1 |= BIT_AP_AHB_USB20_TFHSRES(0x1F);
