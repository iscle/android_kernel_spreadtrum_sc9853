...
static void isp_3dnr_mem_ctrl_cfg(struct isp_mem_ctrl *mem_ctrl,
	void *data)
{
...
static void isp_3dnr_store_cfg(struct isp_3dnr_store *store, void *data)
{
	struct camera_frame *frame = NULL;
...
static void isp_3dnr_crop_cfg(struct isp_3dnr_crop *crop, void *data)
{
	struct camera_frame *frame = NULL;
...
int isp_3dnr_cap_frame_proc(void *isp_handle, void *data)
{
	...
	if (nr3_info->need_3dnr) {
		isp_3dnr_mem_ctrl_cfg(&nr3_info->mem_ctrl, data);
		isp_3dnr_store_cfg(&nr3_info->nr3_store, data);
		isp_3dnr_crop_cfg(&nr3_info->crop, data);
...
int nr3_calculate_for_blending(struct nr3_slice *in,
