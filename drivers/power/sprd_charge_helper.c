...
int sprdchg_read_temp_adc(void)
{
	...
	if (pbat_data->temp_support) {
		while (cnt_adc_swp--) {
			glb_adi_write(ANA_REG_GLB_AUXAD_CTL,
				(cnt_adc_swp & 0x01) ? BIT_AUXAD_NTCAMP_SWP : 0,
				BIT_AUXAD_NTCAMP_SWP);
			/* udealy(50) to ensure write
			 * register AUXAD_CTL success
			 */
			udelay(50);
			adc_value = sprdchg_read_ntc_vol
				(pbat_data->channel_temp);
			sum_adc += adc_value;
		}
		return sum_adc >> 1;
	}
