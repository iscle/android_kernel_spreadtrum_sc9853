...
static int of_parse_sr_table(struct device_node *node,
				struct panel_info *panel)
{
	uint16_t *p;
	int rc, count;

	count = of_property_count_u16_elems(node, "sr-table");
	if (count < 0)
		return count;
	...
	p = kzalloc(sizeof(*p) * count, GFP_KERNEL);
	if (!p)
		return -ENOMEM;
	rc = of_property_read_u16_array(node, "sr-table", p, count);
	if (rc) {
		pr_err("get sr-table failed\n");
		kfree(p);
		return rc;
	}

	panel->sr_count = count / 2;
	panel->sr_table = (uint32_t *)p;
...
static struct panel_info *of_parse_panel(struct device_node *panel_node)
{
	...
	of_parse_sr_table(panel_node, panel);
