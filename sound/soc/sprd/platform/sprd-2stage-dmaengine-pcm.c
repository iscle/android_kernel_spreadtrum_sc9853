...
static uint32_t pcm_alloc_dma_linklist_cfg_ddr(struct snd_pcm_substream
					       *substream,
					       u32 linklist_node_size)
{
...
static uint32_t pcm_free_dma_linklist_cfg_ddr(struct snd_pcm_substream
					      *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct sprd_runtime_data *sprd_rtd = runtime->private_data;
...
static int pcm_alloc_dma_linklist_cfg_ddr_s2_2(struct snd_pcm_substream
					       *substream,
				  ...
				  u32 linklist_node_size2)
{
	...
	for (i = 0; i < chan_cnt; i++) {
		...
		pr_info("%s chan[%d] dma_config ptr=%p, size=%#x dma_pdata=%p, size_pdata =%#x\n",
		     __func__, i, sprd_rtd->dma_cb_ptr[i],
...
static int sprd_snd_platform_probe(struct snd_soc_platform *platform)
{
	...
	struct regmap *pmu_com_apb_gpr = NULL;
