...
static void cpp_module_reset(struct cpp_device *dev)
{
	if (!dev) {
...
static void cpp_scale_reset(struct cpp_device *dev)
{
	if (!dev) {
		pr_err("fail to get valid input ptr\n");
		return;
...
static void cpp_rot_reset(struct cpp_device *dev)
{
	if (!dev) {
		pr_err("fail to get valid input ptr\n");
		return;
...
static void cpp_dma_reset(struct cpp_device *dev)
{
	if (!dev) {
		pr_err("fail to get valid input ptr\n");
		return;
...
static void cpp_sc_reg_trace(struct scale_drv_private *p)
{
...
static void cpp_rot_reg_trace(struct rot_drv_private *p)
{
#ifdef SCALE_DRV_DEBUG
	unsigned long addr = 0;
...
static void cpp_dma_reg_trace(struct dma_drv_private *p)
{
#ifdef SCALE_DRV_DEBUG
	unsigned long addr = 0;
...
static int cpp_module_enable(struct cpp_device *dev)
{
...

	{
		...
		cpp_module_reset(dev);

		reg_awr(dev, CPP_MMU_EN, (0xfffffffe));  /* MMU EN */
		reg_owr(dev, MMU_PPN_RANGE1, (0xfff)); /* MMU PPN RANGE1 */
		reg_owr(dev, MMU_PPN_RANGE2, (0xfff)); /* MMU PPN RANGE2 */
	}
	mutex_unlock(&dev->lock);

	pr_info("cpp_module_enable ok\n");
			...
			unsigned long arg)
{
	...
	case SPRD_CPP_IO_START_ROT:
	{
		...
		if (!cpp_rot_is_end(&rot_parm)) {
			...
			if (rtn == 0) {
				cpp_rot_reg_trace(&rotif->drv_priv);
	...
	case SPRD_CPP_IO_SCALE_CAPABILITY: /* need to Implement */
	{
		if (!arg) {
	...
	case SPRD_CPP_IO_START_SCALE:
	{
		...
		rtn = wait_for_completion_timeout(&scif->done_com,
						...
						(SCALE_TIMEOUT));
		if (rtn == 0) {
			cpp_sc_reg_trace(&scif->drv_priv);
	...
	case SPRD_CPP_IO_START_DMA:
	{
		...
		rtn = wait_for_completion_timeout(&dmaif->done_com,
						...
						(DMA_TIMEOUT));
		if (rtn == 0) {
			cpp_dma_reg_trace(&dmaif->drv_priv);
