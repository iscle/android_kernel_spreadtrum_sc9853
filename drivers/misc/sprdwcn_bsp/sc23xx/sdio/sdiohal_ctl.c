...
char test_buf[1024];
char buf[SDIOHAL_WRITE_SIZE + PUB_HEAD_RSV];
char buf_list[CHANNEL_RX_BASE][SDIOHAL_WRITE_SIZE + PUB_HEAD_RSV];
char tp_tx_buf[TP_TX_BUF_CNT][TP_TX_BUF_LEN + PUB_HEAD_RSV];
...
long int sdiohal_log_level = SDIOHAL_NORMAL_LEVEL | SDIOHAL_MBUF_LIST_LEVEL |
			     SDIOHAL_DATA_LEVEL | SDIOHAL_PERF_LEVEL;
...
static int sdiohal_simple_test_tx(size_t count)
{
	...
	if (!sprdwcn_bus_list_alloc(at_tx_ops.channel,
		...
		if (tx_debug_num >= 4) {
			sdiohal_info("%s tx_debug_num=%d head:%p\n",
			...
			sdiohal_info("%s tx_debug_num=%d < 5\n",
...
static void sdiohal_throughput_tx_compute_time(void)
{
	...
	if (tp_tx_cnt % 500 == 0) {
		...
		sdiohal_info("tx -> times(500c) is %lld\n",
			times_count);
		...
		const char __user *user_buf, size_t count, loff_t *pos)
{
	...
	if (strncmp(buf + PUB_HEAD_RSV, "switch_irq", 10) == 0) {
		...
		sdiohal_info("%s switch irq to [%d][%s]\n",
