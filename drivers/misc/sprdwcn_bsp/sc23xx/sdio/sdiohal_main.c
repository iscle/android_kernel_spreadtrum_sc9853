...
int sdiohal_sdio_pt_write(unsigned char *src, unsigned datalen)
{
	struct sdiohal_data_t *p_data = sdiohal_get_data();
	int ret = 0;
	...
	if (datalen % 4 != 0) {
		sdiohal_err("%s datalen not alligned to 4 byte\n", __func__);
		return -1;
	}
...
int sdiohal_sdio_pt_read(unsigned char *src, unsigned datalen)
{
	struct sdiohal_data_t *p_data = sdiohal_get_data();
	int ret = 0;
	struct timespec tm_begin, tm_end;
			    ...
			    void *buf, unsigned int len)
{
	...
	sdio_claim_host(p_data->sdio_func[FUNC_1]);
	ret = sdio_memcpy_toio_no_incraddr(p_data->sdio_func[FUNC_1],
					   SDIOHAL_DT_MODE_ADDR, buf, len);
	sdio_release_host(p_data->sdio_func[FUNC_1]);
	if (ret != 0)
		sdiohal_abort();
	sdiohal_op_leave();
			   ...
			   unsigned int len)
{
	...
	sdio_claim_host(p_data->sdio_func[FUNC_1]);
	ret = sdio_memcpy_fromio_no_incraddr(p_data->sdio_func[FUNC_1],
...
int sdiohal_scan_card(void)
{
	...
	if (p_data->sdio_dev_host == NULL) {
		sdiohal_err("sdio_sdio_rescan get host failed!\n");
		return -1;
	}
...
int sdiohal_enable_slave_irq(void)
{
	struct sdiohal_data_t *p_data = sdiohal_get_data();
	int err;
...
static int sdiohal_get_dev_func(struct sdio_func *func)
{
	...
	sdiohal_info("func num is %d!!!", func->num);
...
static struct mmc_host *sdiohal_dev_get_host(struct device_node *np_node)
{
	...
	if (host == NULL) {
		sdiohal_err("sdio dev get drvdata failed!!!");
		return NULL;
	}
...
static int sdiohal_parse_dt(void)
{
	...
	if (sdio_node == NULL) {
		sdiohal_err("get sdhci-name failed");
		return -1;
	}
	...
	const struct sdio_device_id *id)
{
	...
	sdiohal_info("%s Enter\n", __func__);
	sdiohal_info("func_class: func->class=%x\n", func->class);
	sdiohal_info("vendor: 0x%04x\n", func->vendor);
	sdiohal_info("device: 0x%04x\n", func->device);
	sdiohal_info("func_num: 0x%04x\n", func->num);
	...
	sdiohal_info("rescan callback:%p\n", scan_card_notify);
...
int sdiohal_init(void)
{
	...
	sdiohal_info("sdiohal_init entry\n");
