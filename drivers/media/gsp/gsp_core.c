...
void gsp_core_work(struct gsp_core *core)
{
	...
	ret = queue_kthread_work(&core->kworker, &core->trigger);
	if (ret == false)
...
void gsp_core_trigger(struct kthread_work *work)
{
	...
	if (ret)
		queue_kthread_work(&core->kworker, &core->recover);
}
...
void gsp_core_release(struct kthread_work *work)
{
	...
	gsp_core_state_set(core, CORE_STATE_TRIGGER);
	success = queue_kthread_work(&core->kworker, &core->trigger);
	if (success == false)
		GSP_WARN("queue trigger work failed\n");
...
int gsp_core_init(struct gsp_core *core)
{
	...
	init_kthread_worker(&core->kworker);

	init_kthread_work(&core->trigger, gsp_core_trigger);
	init_kthread_work(&core->release, gsp_core_release);
	init_kthread_work(&core->recover, gsp_core_recover);
