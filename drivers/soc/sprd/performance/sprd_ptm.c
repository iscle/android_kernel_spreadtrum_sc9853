...
sprd_ptm_legacy_time_handler(struct hrtimer *timer)
{
	...
	bm_info[sdev->mode_info.legacy.bm_buf_write_cnt].t_stop = (u32)ts_val;
	bm_info[sdev->mode_info.legacy.bm_buf_write_cnt].count = num++;
	/* it should clear ptm eb before read ptm data */
	sprd_ptm_set_enable(sdev, false);
	for (chn = 0; chn < sdev->pub_chn; chn++) {
		bm_info[sdev->mode_info.legacy.bm_buf_write_cnt].
			perf_data[chn][0] =
...
static int sprd_ptm_legacy_start(struct sprd_ptm_dev *sdev, u32 cir)
{
	...
	if (!sdev->mode_info.legacy.per_buf)
		return -ENOMEM;
	hrtimer_init(&sdev->mode_info.legacy.timer, CLOCK_REALTIME,
				HRTIMER_MODE_REL);
