...
static inline void sprd_hsphy_reset_core(struct sprd_hsphy *phy)
{
	...
	/* Reset PHY */
	msk1 = BIT_AP_AHB_OTG_UTMI_SOFT_RST | BIT_AP_AHB_OTG_SOFT_RST;
	msk2 = BIT_AON_APB_OTG_PHY_SOFT_RST;
...
static int sprd_hostphy_set(struct usb_phy *x, int on)
{
	...
	if (on) {
		msk = BIT_ANLG_PHY_G4_ANALOG_USB20_UTMIOTG_IDDG;
		regmap_update_bits(phy->ana_g4,
			REG_ANLG_PHY_G4_ANALOG_USB20_IDDG, msk, 0);

		msk = BIT_ANLG_PHY_G4_DBG_SEL_ANALOG_USB20_USB20_DMPULLDOWN |
			BIT_ANLG_PHY_G4_DBG_SEL_ANALOG_USB20_USB20_DPPULLDOWN;
		regmap_update_bits(phy->ana_g4,
			REG_ANLG_PHY_G4_ANALOG_USB20_REG_SEL_CFG_0,
			msk, msk);

		msk = BIT_ANLG_PHY_G4_ANALOG_USB20_USB20_DMPULLDOWN |
			BIT_ANLG_PHY_G4_ANALOG_USB20_USB20_DPPULLDOWN;
		regmap_update_bits(phy->ana_g4,
			REG_ANLG_PHY_G4_ANALOG_USB20_USB20_UTMI_CTL2,
			msk, msk);

		msk = BIT_ANLG_PHY_G4_ANALOG_USB20_USB20_RESERVED(0x200);
		regmap_update_bits(phy->ana_g4,
			REG_ANLG_PHY_G4_ANALOG_USB20_USB20_UTMI_CTL1,
			msk, msk);
	} else {
		reg = msk = BIT_ANLG_PHY_G4_ANALOG_USB20_UTMIOTG_IDDG;
		regmap_update_bits(phy->ana_g4,
...
static void sprd_hsphy_emphasis_set(struct usb_phy *x, bool enabled)
{
	...
	msk = BIT_ANLG_PHY_G4_ANALOG_USB20_USB20_TUNEHSAMP(2);
	if (enabled)
		regmap_update_bits(phy->ana_g4,
...
static int sprd_hsphy_init(struct usb_phy *x)
{
	...
	value |= BIT_AP_AHB_OTG_VBUS_VALID_PHYREG;
	writel_relaxed(value, phy->base + REG_AP_AHB_OTG_PHY_TEST);

	reg = msk = BIT_ANLG_PHY_G4_ANALOG_USB20_USB20_VBUSVLDEXT;
	regmap_update_bits(phy->ana_g4,
		REG_ANLG_PHY_G4_ANALOG_USB20_USB20_UTMI_CTL1, msk, reg);

	reg = msk = BIT_ANLG_PHY_G4_ANALOG_USB20_USB20_DATABUS16_8;
	regmap_hwlock_update_bits(phy->ana_g4,
		REG_ANLG_PHY_G4_ANALOG_USB20_USB20_UTMI_CTL1, msk, reg);

	/* for SPRD phy utmi_width sel */
	value = readl_relaxed(phy->base + REG_AP_AHB_OTG_PHY_CTRL);
	value |= BIT_AP_AHB_UTMI_WIDTH_SEL;
	writel_relaxed(value, phy->base + REG_AP_AHB_OTG_PHY_CTRL);

	reg = msk = BIT_ANLG_PHY_G4_ANALOG_USB20_USB20_TUNEHSAMP(2);
	regmap_update_bits(phy->ana_g4,
...
static void sprd_hsphy_shutdown(struct usb_phy *x)
{
	...
	regmap_update_bits(phy->hsphy_glb, REG_AON_APB_PWR_CTRL,
		BIT_AON_APB_USB_ISO_SW_EN, BIT_AON_APB_USB_ISO_SW_EN);

	regmap_update_bits(phy->hsphy_glb, REG_AON_APB_APB_EB2,
		BIT_AON_APB_OTG_REF_EB, 0);
