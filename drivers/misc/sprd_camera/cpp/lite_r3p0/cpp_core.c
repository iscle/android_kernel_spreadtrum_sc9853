...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "CPP_CORE: %d %d %s: " fmt, current->pid, __LINE__, __func__
			...
			unsigned long arg)
{
	...
	case SPRD_CPP_IO_START_DMA:
	{
		dmaif = dev->dmaif;
		if (!dmaif) {
			pr_err("dma start:dmaif is null!\n");
			return -EFAULT;
		}
