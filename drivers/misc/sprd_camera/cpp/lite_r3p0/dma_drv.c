...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "DMA_DRV: %d %d %s: " fmt, current->pid, __LINE__, __func__
					...
					struct dma_drv_private *p)
{
	...
	if ((parm->input_size.w % 4 != 0) || (parm->input_size.h % 1 != 0)) {
		pr_err("width and height not aligned:%u, %u",
			parm->input_size.w, parm->input_size.h);
		return -EINVAL;
	}

	if ((p->src_offset_x % 2 != 0) || (p->dst_offset_x % 8 != 0)) {
		pr_err("src and dest offset x not aligned:%u, %u",
	...
	if ((p->src_pitch % 8 != 0)
		...
		pr_err("src pitch not aligned:%u", p->src_pitch);
		return -EINVAL;
	}

	if ((p->dst_pitch % 8 != 0)
		|| (p->dst_pitch < (parm->input_size.w + p->dst_offset_x))) {
		pr_err("dest pitch not aligned:%u", p->src_pitch);
		return -EINVAL;
	}
...
static int cpp_dma_set_parm(struct dma_drv_private *p)
{
	...
	if ((p->dma_src_addr & DMA_ADDR_ALIGN) ||
		...
		pr_err("src and dst addr not aligned\n");
		return -EINVAL;
	}
