...
struct sprd_adf_hwlayer corner_layer_top = {
	.n_planes = 1,
	.interface_id = 0,
	.compression = 0,
...
struct sprd_adf_hwlayer corner_layer_bottom = {
	.n_planes = 1,
	.interface_id = 0,
	.compression = 0,
	.format = DRM_FORMAT_RGBA8888,
...
static int sprd_corner_create(int width, int radius)
{
	...
	layer_top = (unsigned int *)kzalloc(buf_size, GFP_KERNEL);
	layer_bottom = (unsigned int *)kzalloc(buf_size, GFP_KERNEL);
...
int sprd_corner_hwlayer_init(int panel_height, int panel_width, int corner_radius)
{
	...
	corner_layer_top.iova_plane[0] = (u32)virt_to_phys(layer_top);
	...
	corner_layer_bottom.iova_plane[0] = (u32)virt_to_phys(layer_bottom);
