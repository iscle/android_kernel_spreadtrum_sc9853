...
static void record_cpu_usage(void)
{
	...
	rtc_time_to_tm(recorder[pos].timestamp.ts_start.tv_sec,
	...
	rtc_time_to_tm(recorder[pos].timestamp.ts_end.tv_sec,
...
static void print_summary(struct seq_file *p, void *v, unsigned long bufid)
{
	...
	if (bufid == bufferid(global_index)) {
		...
		rtc_time_to_tm(recorder[bufid].timestamp.ts_start.tv_sec,
			       &recorder[bufid].timestamp.tm_start);
		rtc_time_to_tm(recorder[bufid].timestamp.ts_end.tv_sec,
