	...
	void *isp_handle)
{
	...
	struct isp_nr3_param *nr3_param = NULL;
	...
	nr3_param = &module_info->isp_path[0].nr3_param;
...
int isp_k_cfg_3dnr(struct isp_io_param *param,
	struct isp_k_block *isp_k_param, uint32_t idx, void *isp_handle)
{
	int ret = 0;
	...
	case ISP_PRO_3DNR_UPDATE_PRE_PARAM:
		ret = isp_k_3dnr_block(param, idx, isp_handle);
		break;
	default:
