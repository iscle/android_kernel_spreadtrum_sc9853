...
int isp_cfg_map_init(struct isp_cfg_ctx_desc *cfg_ctx)
{
	uint32_t i = 0, idx = 0;
...
int isp_cfg_ctx_buf_init(struct isp_cfg_ctx_desc *cfg_ctx, uint32_t flag)
{
	...
	for (scene_id = ISP_SCENE_PRE; scene_id < ISP_SCENE_MAX; scene_id++) {
		...
		if (flag == 1)
			...
			isp_cfg_ctx_addr[id][scene_id] =
				(unsigned long)&cfg_ctx->temp_cfg_buf[0] +
				ISP_REG_SIZE * scene_id;

		cfg_buf_p->cur_buf_id = CFG_BUF_SHADOW;

		pr_info("cmd_buf_vir 0x%lx, cmd_buf_phy 0x%lx\n",
			(unsigned long)
			cfg_buf_p->cmd_buf[CFG_BUF_SHADOW].vir_addr,
			(unsigned long)
			cfg_buf_p->cmd_buf[CFG_BUF_SHADOW].phy_addr);
	}

	return 0;
}

void isp_cfg_start_isp(struct isp_cfg_ctx_desc *cfg_ctx)
{
...
int isp_config_cfg_block(struct isp_cfg_ctx_desc *cfg_ctx)
{
	...
	pr_debug("Enter\n");
	if (!cfg_ctx) {
		pr_err("can't init empty cfg_ctx\n");
		return -EFAULT;
	}
...
int isp_update_cfg_buf(struct isp_cfg_ctx_desc *cfg_ctx)
{
	int ret = ISP_RTN_SUCCESS;
	...
	if (scene_id == ISP_SCENE_PRE || scene_id == ISP_SCENE_CAP) {
		...
		if (unlikely(!IS_ALIGNED(work_buf_paddr, ISP_REG_SIZE))) {
			pr_err("phy addr should be aligned with 256KB\n");
			return -EINVAL;
		}
		...
		pr_debug("shadow_vaddr:0x%p, work_vaddr:0x%p, work_paddr:0x%lx, buf_id %d\n",
			shadow_buf_vaddr, work_buf_vaddr,
			work_buf_paddr, work_buf_id);
...
int isp_reset_cfg_buf(struct isp_cfg_ctx_desc *cfg_ctx)
{
	int ret = ISP_RTN_SUCCESS;
	void *shadow_buf_vaddr = NULL;
	enum isp_id id = ISP_ID_0;
	...
	shadow_buf_vaddr = (void *)isp_cfg_ctx_addr[id][0];
	memset(shadow_buf_vaddr, 0, ISP_REG_SIZE);
	shadow_buf_vaddr = (void *)isp_cfg_ctx_addr[id][1];
	memset(shadow_buf_vaddr, 0, ISP_REG_SIZE);

	return ret;
}

int isp_cfg_ctx_init(struct isp_cfg_ctx_desc *cfg_ctx, uint32_t idx)
{
	int ret = ISP_RTN_SUCCESS;
	...
	/*reset the cmd buf using default values*/
#if 0
	for (idx = 0; idx < ISP_ID_MAX; idx++) {
		for (scene_id = 0; scene_id < ISP_SCENE_MAX; scene_id++) {
			for (buf_num = 0; buf_num < ISP_CTX_BUF_NUM;
					buf_num++) {
				vir_addr =
				cfg_ctx->buf[idx][scene_id][buf_num].vir_addr;
				memset(vir_addr, 0, ISP_REG_SIZE);
				memcpy(vir_addr, ISP_REG_DEFAULT, ISP_REG_SIZE);
			}
		}
	}
#endif
...
int isp_cfg_ctx_deinit(struct isp_cfg_ctx_desc *cfg_ctx)
{
	int ret = ISP_RTN_SUCCESS;
