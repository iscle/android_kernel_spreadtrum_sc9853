...
#define SLICE_WIDTH_TEMP                2048
#define SLICE_SCL_CAP_WIDTH_MAX        2592
#define SLICE_SCL_VID_WIDTH_MAX        2304
#define SLICE_WIDTH_DUAL_CAM_MAX  1600
...
#define SCL_OVERLAP_DOWNv              (SCL_VER_MAX_TAP / 2 + 1)
...
int fmcu_push_back(uint32_t *p, uint32_t addr,
...
uint32_t noisefilter_24b_shift8(uint32_t seed, uint32_t *data_out)
{
...
void slice_noisefilter_seeds(uint32_t image_width, uint32_t seed0,
	uint32_t *seed1, uint32_t *seed2, uint32_t *seed3)
{
	uint32_t i = 0;

	*seed1 = noisefilter_24b_shift8(seed0, NULL);
	*seed2 = seed0;

	for (i = 0; i < image_width; i++)
		*seed2 = noisefilter_24b_shift8(*seed2, NULL);

	*seed3 = noisefilter_24b_shift8(*seed2, NULL);
}
...
static int get_slice_size_info(struct slice_param_in *in_ptr,
	uint32_t *h, uint32_t *w, enum isp_scene_id sid)
{
	int rtn = 0;
	uint32_t tempw = 0;
	...
	dual_cam_mode = sprd_isp_get_dual_cam_mode();
	id = sprd_dcam_get_chip_id();
	if (sid == ISP_SCENE_PRE)
		output = &in_ptr->store_frame[SLICE_PATH_PRE].size;
	else if (sid == ISP_SCENE_CAP)
		output = &in_ptr->store_frame[SLICE_PATH_CAP].size;

#if 0
	if (input->width == output->width &&
		input->height == output->height) {
		*w = (input->width > SLICE_WIDTH_MAX) ?
			SLICE_WIDTH_MAX : input->width;
	} else {
		if (output->width > SLICE_SCL_CAP_WIDTH_MAX) {
			tempw = (output->width + SLICE_SCL_CAP_WIDTH_MAX
				- 1) / SLICE_SCL_CAP_WIDTH_MAX;
			*w = input->width / tempw;
			*w = (*w > SLICE_WIDTH_MAX) ?
			SLICE_WIDTH_MAX : *w;
		} else {
	...
	if (output->width > SLICE_SCL_CAP_WIDTH_MAX) {
		...
		*w = (*w > SLICE_WIDTH_MAX) ?
		...
		if (id == SHARKL3)
			*w = SLICE_WIDTH_TEMP;
	} else if (input->width > SLICE_WIDTH_MAX) {
		tempw = (input->width + SLICE_WIDTH_MAX
			- 1) / SLICE_WIDTH_MAX;
		*w = input->width / tempw;
		*w = (*w > SLICE_WIDTH_MAX) ?
		SLICE_WIDTH_MAX : *w;
	...
	if ((input->width > SLICE_SCL_CAP_WIDTH_MAX) &&
		dual_cam_mode && (id != SHARKL3))
		*w = SLICE_WIDTH_DUAL_CAM_MAX;
#endif
...
static int set_slice_base_info(struct slice_param_in *in_ptr,
	struct slice_base_info *base_info, enum isp_scene_id sid)
{
	int rtn = 0;
	uint32_t i = 0, j = 0;
	...
	rtn = get_slice_size_info(in_ptr, &slice_height, &slice_width, sid);
	if (rtn) {
	...
	pr_info("slice row %d, col %d, num %d\n", slice_total_row,
	...
	struct slice_context_info *cxt)
{
	...
	for (; cur_slice_id < slice_num; cur_slice_id++) {
		...
		pr_info("fetch_info W 0x%x, H 0x%x, format %d\n",
			end_col, end_row, in_ptr->fetch_format);
		switch (in_ptr->fetch_format) {
		case ISP_FETCH_YUV422_3FRAME:
...
static int set_slice_3dnr_mem_ctrl(struct slice_store_path *in_ptr,
	struct slice_context_info *cxt)
{
	int rtn = 0;
	struct slice_base_info *base_info = NULL;
	...
	get_slice_fetch_pitch(&fetch_pitch, in_ptr->format,
		base_info->img_width);
	for (; cur_slice_id < slice_num; cur_slice_id++) {
...
static int set_slice_dispatch_info(struct slice_context_info *cxt)
{
	int rtn = 0;
	uint32_t cur_slice_id, slice_num;
	uint32_t start_col, end_col, start_row, end_row;
...
static int set_slice_nlm_info(struct slice_context_info *cxt)
{
	int rtn = 0;
	uint32_t cur_slice_id, slice_num;
	uint32_t start_col, end_col, start_row, end_row;
	...
	/*TBD:
	 * nlm parameter calc should be make sure.
	 */
	for (; cur_slice_id < slice_num; cur_slice_id++) {
		...
		start_row = base_info->slice_raw_pos_array
			[cur_slice_id].start_row;
		end_row = base_info->slice_raw_pos_array[cur_slice_id].end_row;
		nlm_info->col_center = (end_col - start_col) / 2 + start_col;
		nlm_info->row_center = (end_row - start_row) / 2 + start_row;
	}
exit:
	return rtn;
}
	...
	struct slice_scaler_info *scaler_info, uint32_t scl_bypass)
{
	...
	if (get_slice_store_pitch(&store_pitch, store_frame->format,
...
static int set_slice_3dnr_store_info(struct slice_store_path *in_ptr,
	struct slice_context_info *cxt)
{
	int rtn = 0;
	struct slice_base_info *base_info = NULL;
	...
	if (get_slice_store_pitch(&store_pitch, in_ptr->format,
		base_info->img_width)) {
		pr_err("fail to get slice store pitch\n");
		rtn = -EINVAL;
		goto exit;
...
static int set_slice_3dnr_crop_info(struct slice_context_info *cxt)
{
	int rtn = 0;
	struct slice_base_info *base_info = NULL;
	struct slice_3dnr_mem_ctrl *nr3_mem_ctrl = NULL;
...
static int update_slice_3dnr_mem_ctrl(struct slice_param_in *in_ptr,
	struct slice_context_info *cxt)
{
	int rtn = 0;
	struct slice_base_info *base_info = NULL;
	...
	for (; cur_slice_id < slice_num; cur_slice_id++) {
		...
		rtn = nr3_calculate_for_blending(&nr3_fw_in, &nr3_fw_out);
		if (rtn) {
...
static int set_slice_3dnr_info(struct slice_param_in *in_ptr,
	struct slice_context_info *cxt)
{
	int rtn = 0;
	...
	rtn = update_slice_3dnr_mem_ctrl(in_ptr, cxt);
	if (rtn) {
		pr_err("fail to update slice 3dnr mem ctrl!\n");
		goto exit;
	}

	rtn = set_slice_3dnr_store_info(&slice_in->store_3dnr_frame, cxt);
	if (rtn) {
		pr_err("fail to set slice 3dnr store info!\n");
		goto exit;
	}

	rtn = set_slice_3dnr_crop_info(cxt);
	if (rtn) {
...
static int set_fmcu_cfg(uint32_t *fmcu_buf, uint32_t num,
...
static int set_fmcu_ynr(uint32_t *fmcu_buf,
...
static int set_fmcu_nlm(uint32_t *fmcu_buf,
...
static int set_fmcu_3dnr_mem_ctrl(uint32_t *fmcu_buf,
...
static int set_fmcu_3dnr_store(uint32_t *fmcu_buf,
...
static int set_fmcu_3dnr_crop(uint32_t *fmcu_buf,
	...
	struct slice_context_info *cxt, uint32_t *fmcu_num)
{
	...
	for (slice_id = 0; slice_id < base_info->slice_num; slice_id++) {
		...
		if (work_mode == ISP_CFG_MODE)
			num = set_fmcu_cfg(fmcu_buf, num, scene_id, id);
		num = set_fmcu_fetch(fmcu_buf, fetch_info, num, id);
		num = set_fmcu_dispatch(fmcu_buf,
			dispatch_info, num, id);
#if 0
		num = set_fmcu_postcnr(fmcu_buf, postcnr_info, num, id);
		num = set_fmcu_ynr(fmcu_buf, ynr_info, num, id);
		num = set_fmcu_noisefilter(fmcu_buf,
			noisefilter_info, num, id);
		num = set_fmcu_nlm(fmcu_buf, nlm_info, num, id);
		num = set_fmcu_cfa(fmcu_buf, cfa_info, num, id);
#endif
		if (in_ptr->nr3_info.need_slice) {
			num = set_fmcu_3dnr_mem_ctrl(fmcu_buf,
				nr3_mem_ctrl, num, id);
			num = set_fmcu_3dnr_store(fmcu_buf,
				store_3dnr_info, num, id);
			num = set_fmcu_3dnr_crop(fmcu_buf,
			...
			if (in_ptr->nr3_info.cur_frame_num == 1) {
				...
				cmd = ISP_REG_CFG_RD(in_ptr->com_idx,
					ISP_YUV_3DNR_MEM_CTRL_PARAM0);
				pr_debug("3DNR fmcu_info num 1 cmd=0x%x,com_idx=0x%x\n",
					cmd, in_ptr->com_idx);
				cmd &= (~BIT_23);
				cmd |= (BIT_6 | BIT_12);
				num = fmcu_push_back(&fmcu_buf[num],
					addr, cmd, num);
				addr = ISP_GET_REG(id,
				...
				cmd = ISP_REG_CFG_RD(in_ptr->com_idx,
					ISP_YUV_3DNR_MEM_CTRL_PARAM0);
				pr_debug("3DNR fmcu_info  more than one cmd=0x%x,idx=0x%x\n",
					cmd, in_ptr->com_idx);
		...
		if (work_mode == ISP_CFG_MODE)
			cmd = shadow_done_cmd[id][scene_id];/*TBD*/
		else
	 ...
	 enum isp_scene_id sid)
{
	...
	rtn = set_slice_base_info(in_ptr, &cxt->base_info, sid);
	if (rtn) {
	...
	rtn = set_slice_dispatch_info(cxt);
	if (rtn) {
		pr_err("fail to set slice dispatchYUV info!\n");
		goto exit;
	}

	rtn = set_slice_nlm_info(cxt);
	if (rtn) {
		pr_err("fail to set slice dispatchYUV info!\n");
		goto exit;
	}
	...
	rtn = set_slice_noisefliter_info(cxt, sid);
	if (rtn) {
	...
	if (in_ptr->nr3_info.need_slice) {
		rtn = set_slice_3dnr_info(in_ptr, cxt);
		if (rtn) {
