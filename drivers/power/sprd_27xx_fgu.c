...
#define BAT_FILE_PATH "/productinfo/.battery_file"
...
static int sprdfgu_find_firstpoweron_cap(void)
{
	...
	SPRD_FGU_DEBUG("val:%d,fgu_type:%d\n",
		val, sprdfgu_data.fgu_pdata->fgu_type);
	if (((gpio_get_value(
		...
		SPRD_FGU_DEBUG("poweron soft_ocv:%d\n",
			soft_ocv);
		SPRD_FGU_DEBUG("init_cap:%d\n", cap);
	} else {
		int poci_raw =
			sprdfgu_reg_get(REG_FGU_CLBCNT_QMAXL) << 1;
		u32 pocv_raw =
			fgu_adi_read(REG_FGU_POCV_VAL);
		int poci_curr =
			sprdfgu_adc2cur_ma(poci_raw -
		...
		SPRD_FGU_DEBUG("poci_raw:0x%x,poci_cur:%d\n",
			poci_raw, poci_curr);
		SPRD_FGU_DEBUG("softocv:%d,init_cap:%d\n",
			p_ocv, cap);
	}
