...
{
	unsigned int mask = 0;
	char sleep_str[] = "gnss_sleep ";
	char resume_str[] = "gnss_resume";
	...
	if (gnss_dev.gnss_flag_sleep) {
		...
		gnss_dev.gnss_flag_sleep = false;
		memcpy(gnss_dev.gnss_status, sleep_str, sizeof(sleep_str));
	...
	if (gnss_dev.gnss_flag_resume) {
		...
		gnss_dev.gnss_flag_resume = false;
		memcpy(gnss_dev.gnss_status, resume_str, sizeof(resume_str));
		mask |= POLLIN | POLLRDNORM;
	}
